...
function Executable_C() {
    echo "Checking the 'cwire' executable..."
...
echo "Central's ID used : $identifiant_centrale"
...
# Vérification si le fichier temporaire csv existent
if [ ! -s "tmp/${type_station}_${type_conso}.csv" ]; then
    echo "Error: The file 'tmp/${type_station}_${type_conso}.csv' is empty or could not be generated."
    exit 1
fi

echo "Launch of the C program"
